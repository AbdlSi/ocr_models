...
print(f"[DEBUG] Started PaddleOCR PP-OCRv5 at {datetime.now().isoformat(timespec='seconds')}")
...
result = ocr.predict(r"C:\Users\asus\OneDrive\Desktop\tubitak_sample_docs\benchmark_mock_docs")
combined_results = []
...
combined_output_file = os.path.join(output_dir, "paddleocr_pp_ocrv5_multilingual_output.json")
with open(combined_output_file, "w", encoding="utf-8") as f:
    json.dump(combined_results, f, ensure_ascii=False, indent=2)

print(f"[DEBUG] Saved combined PaddleOCR PP-OCRv5 output to: {combined_output_file}")
print(f"[DEBUG] Finished PaddleOCR PP-OCRv5 at {datetime.now().isoformat(timespec='seconds')}")
